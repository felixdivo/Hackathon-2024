...
def overlay_gripper_on_part(gripper_image, part_image, x, y, angle):
    ...
    rotated_gripper = cvzone.rotateImage(gripper_image, angle=angle, scale=1, keepSize=False)
    ...
    part_image_padded = np.ones((ph + (pad_top + pad_bottom), pw + (pad_left + pad_right), 3), dtype=np.uint8) * 255
    part_image_padded[(pad_top):(pad_top + ph), (pad_left):(pad_left + pw)] = part_image
    ...
    mask_combined = cvzone.overlayPNG(part_image_copy, rotated_gripper, [overlay_x, overlay_y])
    ...
    gripper_mask = cv2.threshold(alpha_channel, 0, 255, cv2.THRESH_BINARY)[1]  # Binary mask for visible regions
    ...
    aligned_gripper_mask[overlay_y:overlay_y + gh, overlay_x:overlay_x + gw] = gripper_mask
...
def main():
    ...
    for index, output_row in outputs.iterrows():
        ...
        if not (output_part == ground_truth_part and output_gripper == ground_truth_gripper):
        ...
        gripper_image = cv2.imread(Path(ground_truth_row["gripper"]), cv2.IMREAD_UNCHANGED)
